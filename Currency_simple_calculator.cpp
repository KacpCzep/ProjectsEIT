...
/*English and polish scirpt */
/* Design and implement classes for a currency calculator - static conversion, data is not updated from the Internet.  */
/*
...
/*   Main class responsible for setting the currency and amount of money to convert.    */
...
/* User chooses currency in which he will operate. */
void Kantor::waluta_we() //Pobieranie od uzytkownika waluty, z ktorej wymienia pieniadze
{
    cout << "Simple currency calcaulator. Last data update: 16th December 2021.  \nCurrency: -\n";
    cout << "(P) Zloty, (E) Euro, (U) US dollar \n";
    cout << "(G) British pound, (R) Russian rubel \n";
    cout << "(K) Croatian kuna, (C) Swiss franc \n";
    cout << "To exit program press (Q)\n\n";
    cout << "Pick currency you own: ";

    
    string m_waluta;
    start:  
    cin >> m_waluta;
	/* Protectiong against entering 2 or more characters. */
    //Ochrona przed wpisaniem 2 i wiecej znakow
    if (m_waluta[1] != '\0') {
        cout << "Pick only one letter.\nChoose your currency: ";
        goto start;
    }
    ...
    case 'C':
        ...
        break;
    case 'Q':
        cout << "Thank you for using the converter. Exiting...";
        exit(0);
    default:
        cout << "Pick proper currency (letter): ";
        goto start;
    }
	/*  Gather currency from user (which he owns). */
    get_waluta_pocz(); //Uzyskanie waluty od uzytkownika
}
	/*  Gather currency from user (which he wants to convert to). */
void Kantor::waluta_wy() //Pobieranie od uzytkownika waluty na ktora wymienia pieniadze
{
    cout << "Choose currency you wish to buy: ";
    string m_walut;
start:
    cin >> m_walut;
    ...
    /* Check if user didn't enter two exact currencies */
    ...
    case 'P':
        if (m_waluta_pocz == 'P') {
            cout << "You picked PLN as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'P';
        break;
    case 'E':
        if (m_waluta_pocz == 'E') {
            cout << "You picked Euro as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'E';
        break;
    case 'U':
      if (m_waluta_pocz == 'U') {
            cout << "You picked USD as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'U';
        break;
    case 'G':
      if (m_waluta_pocz == 'G') {
            cout << "You picked GBP as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'G';
        break;
    case 'R':
      if (m_waluta_pocz == 'R') {
            cout << "You picked RUB as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'R';
        break;
    case 'K':
      if (m_waluta_pocz == 'K') {
            cout << "You picked KN as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'K';
        break;
    case 'C':
      if (m_waluta_pocz == 'C') {
            cout << "You picked CHF as both currencies. Enter the other currency you wish to buy: ";
            goto start;
        }
        m_waluta_konc = 'C';
        break;
...
{
	/* Starting as string, to eventually project into intiger. */
    ...
    for (int i = 0; i < kasa_na_kwote.size(); i++) {
        if (isdigit(kasa_na_kwote[i])) {
            //isdigit checks if value is digit
            //atoi - conversion string to int
            kwota = atoi(&kasa_na_kwote[i]);
            return set_kwota(kwota);
        }
        else {
            cout << "You entered wrong data. Please try again.\n";
            goto start;
        }
    }
}
/*  Main method to calculate the money. */
void Kantor::wymiana() //Glowna funkcja, w ktorej odbywa sie konwersja walut
{
    ...
    /*  Added time markers which will differ the transactions. */
...
int main() {
  Kantor klient;
    //Infinite loop. To quit, anywhere in the program enter 'Q' or 'q'.
  for (;;) {
    klient.waluta_we();
    klient.waluta_wy();
    klient.pieniadze();
